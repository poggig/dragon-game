...
class Ent{
  ...
  // Snap entity to the nearest STANDABLE surface. Handles both cases:
  // spawned in open air (scan down for a solid row with body headroom) and
  // spawned INSIDE terrain (rise to the top of that solid block) — Lv4's
  // balcony spawn buried the whole party before the rise pass existed.
  snapGnd(tm){
    const bx1=Math.floor((this.x+4)/T),bx2=Math.floor((this.x+this.w-4)/T);
    const bodyRows=Math.ceil(this.h/T);
    let feet=Math.max(0,Math.floor((this.y+this.h-1)/T));
    if(tm.sol(bx1,feet)||tm.sol(bx2,feet)){
      // Embedded in solid: climb to the surface of this block and stand on it
      let y=feet;
      while(y>0&&(tm.sol(bx1,y)||tm.sol(bx2,y)))y--;
      this.y=(y+1)*T-this.h;this.gnd=true;this.vy=0;return;
    }
